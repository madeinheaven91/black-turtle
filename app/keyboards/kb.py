...
reg_group_or_teacher_builder.add(
    InlineKeyboardButton(text="Группы", callback_data="reg_group")
)
reg_group_or_teacher_builder.add(
    InlineKeyboardButton(text="Преподавателя", callback_data="reg_teacher")
)
reg_group_or_teacher_kb = reg_group_or_teacher_builder.as_markup(resize_keyboard=True)


reg_cancel_kb = InlineKeyboardMarkup(
    inline_keyboard=[[InlineKeyboardButton(text="Отмена", callback_data="reg_cancel")]]
)
